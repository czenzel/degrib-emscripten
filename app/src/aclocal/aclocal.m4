...
#####
# Set @MINIZIP32@ variable.
#####
AC_DEFUN([SET_MINIZIP32],
[
  MINIZIP32=""
  case $host in
    *-*-cygwin*|*-*-mingw*|*-*-msys*)
...
[
  AC_ARG_WITH([halo],
    [AS_HELP_STRING([--with-halo], [enable drawing to image library.])],
    [],
    [with_halo=no])
  AS_IF([test "x$with_halo" != xno],
    [
      AC_SUBST([HALO_DEF],"-DHALO")
  # Single quotes are the key to the following.
      ...
      case $host in
        ...
        *)
          AC_SUBST([HALO_LIBDEP],'$(top_srcdir)/halo/libhalo8.4.so')
          AC_SUBST([HALO_STDLIB],'-L$(top_srcdir)/halo -lhalo8.4');;
      esac
    ])
])
...
# Deal with Tcl shared libraries
    ...
    TCL_LIBS="-L$TCL_PREFIX/lib -ltcl$TCL_VERSION -ltclstub$TCL_VERSION"
    TK_LIBS="-L$TK_PREFIX/lib -ltk$TCL_VERSION -ltkstub$TCL_VERSION"
    AC_SUBST([TK_LIBS])
    AC_SUBST([TCL_LIBS])
  fi
])
